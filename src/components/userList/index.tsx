...
export function UserList() {
    ...
    const fetcher = async (url) => await axios.get(url).then((res) => {setUsuarios(res.data)})
    const { data, error } = useSWR('https://random-persons.herokuapp.com/users', fetcher);
            ...
            <TableContainer width ="100%" bg = "gray.100" padding = "1rem 2rem" >
                ...
                <Table variant='simple'>
                    ...
                    { 
                        ...
                        usuarios.data.filter(Filtered).slice(pop[0], pop[1]).map( ( element , index) => (
                            <Tr key={index}>
                                <Td>{element.name}</Td>
                                <Td>{element.age}</Td>
                            </Tr>
                    ...
                    <Tr>
                                <Td>
                                    <Flex justify="flex-start" padding-top="10">
                                        <Button colorScheme="teal" onClick={prev} isDisabled={prevState}>Anterior</Button>
                                    </Flex>
                                </Td>
                                <Td>
                                    <Flex justify="flex-end" padding-top="10">
                                        <Button colorScheme="teal" onClick={next} isDisabled={nextState}>Próximo</Button>
                                    </Flex>
                                </Td>
                    </Tr>
